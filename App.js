...
export default function App() {
  ...
  const imageOrigin = { 
    x: Dimensions.get('window').width / 2 - (Dimensions.get('window').width * 0.9 / 2),   
    y:Dimensions.get('window').height / 2 - (Dimensions.get('window').height * 0.6 / 2) 
  }
  ...
  const forceSwipe = (direction) => {
    ...
    if(direction === 'top'){
      swipeDirection = { x: 0, y: -Dimensions.get('window').height * 2 }
    }else{
      swipeDirection = direction === 'right' ? { x: Dimensions.get('window').height, y: -Dimensions.get('window').height } : { x: -Dimensions.get('window').height, y: -Dimensions.get('window').height }
    }
  ...
  return (
    <View style={styles.container}>
      <Animated.View {...panResponder.panHandlers} style={[styles.imageContainer,getCardStyle()]}>
